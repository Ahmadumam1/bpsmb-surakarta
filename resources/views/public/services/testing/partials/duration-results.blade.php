    ...
    <table class="w-full min-w-[880px] text-left">
        <thead class="sticky top-0 z-10 bg-[#08236f] text-white">
            <tr>
                ...
                <th class="px-5 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Karakteristik Uji</th>
                <th class="w-[260px] px-5 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Kategori</th>
                <th class="w-[190px] px-5 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Lama Pengujian</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-black/10">
            @if ($testingDurations->isEmpty())
                <tr>
            ...
            @else
                @foreach ($testingDurations as $item)
                    <tr class="transition odd:bg-white even:bg-[#f8fafc] hover:bg-[#fff8e1]">
                        <td class="px-5 py-4 align-top text-sm font-semibold text-[#71717a]">
                            {{ $testingDurations->firstItem() + $loop->index }}
                        </td>
                        <td class="px-5 py-4 align-top">
                            <p class="text-[15px] font-semibold leading-6 text-[#1f2937]">{{ $item->name }}</p>
                        </td>
                        <td class="px-5 py-4 align-top">
    ...
    @else
        @foreach ($testingDurations as $item)
            <div class="rounded-[8px] border border-black/10 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                ...
                <p class="mt-3 text-[15px] font-semibold leading-6 text-[#1f2937]">{{ $item->name }}</p>
            </div>
        @endforeach
    @endif
</div>
