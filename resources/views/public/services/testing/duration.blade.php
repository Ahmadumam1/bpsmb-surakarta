...
@section('title', 'Lama Pengujian | BPSMB Surakarta')
@section('meta_description', 'Informasi estimasi lama pengujian Laboratorium Penguji BPSMB Surakarta berdasarkan karakteristik uji.')
            ...
            <div class="flex items-center">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-3 max-w-2xl text-[13px] leading-6 text-[#4b5563] sm:mt-5 sm:text-[16px] sm:leading-7">
                        Estimasi waktu penyelesaian pengujian berdasarkan karakteristik uji di Laboratorium Penguji BPSMB
                        Surakarta.
                    </p>
        ...
        <div class="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
            <div id="testing-duration-app"
                ...
                <div class="border-b border-black/10 p-4 sm:p-5">
                    <div class="grid gap-3 lg:grid-cols-[1fr_320px] lg:items-center">
                        <label class="relative block">
                            <span class="sr-only">Cari karakteristik uji</span>
                            ...
                            <input type="search" data-testing-search value="{{ $search }}" placeholder="Cari karakteristik uji..."
                        ...
                        <label class="block">
                            ...
                            <select data-testing-filter
                                class="h-12 w-full rounded-[8px] border border-black/10 bg-[#f8fafc] px-4 text-[14px] font-semibold text-[#1f2937] outline-none transition focus:border-[#08236f] focus:bg-white focus:ring-4 focus:ring-[#08236f]/10">
                    ...
                    <p class="mt-3 text-sm font-medium text-[#71717a]" data-testing-count>
                        Menampilkan 0 data.
                    </p>
                </div>

                <div data-testing-results>
                    <div class="service-table-compact overflow-x-auto">
                        <table class="w-full min-w-[500px] table-fixed text-left sm:min-w-[560px]">
                            <thead class="sticky top-0 z-10 bg-[#08236f] text-white">
                                <tr>
                                    <th class="w-9 px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">No</th>
                                    <th class="w-[220px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em] sm:w-[280px]">Karakteristik Uji</th>
                                    <th class="w-[120px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Kategori</th>
                                    <th class="w-[120px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Lama Pengujian</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-black/10" data-testing-table-body></tbody>
                        </table>
                    </div>

                    <div class="hidden" data-testing-card-list></div>
                </div>

                <div class="border-t border-black/10 px-4 py-4 sm:px-5">
                    <div class="flex flex-nowrap items-center justify-start gap-1.5 overflow-x-auto sm:justify-center sm:gap-2" data-testing-pagination></div>
                </div>
            </div>

            @include('public.services.partials.related-services', ['current' => 'testing-duration'])

            <div class="mt-5 grid gap-3 rounded-[8px] border border-black/10 bg-[#08236f] p-3 text-white shadow-[0_12px_32px_rgba(8,35,111,0.14)] sm:mt-6 sm:gap-4 sm:p-5 sm:shadow-[0_20px_56px_rgba(8,35,111,0.16)] lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                    <p class="text-[15px] font-semibold leading-snug sm:text-[18px]">Butuh kepastian jadwal pengujian?</p>
                    <p class="mt-1.5 max-w-2xl text-[12px] leading-5 text-white/72 sm:text-[13px] sm:leading-6">
                        Hubungi petugas layanan untuk konfirmasi estimasi berdasarkan jumlah sampel dan parameter uji.
                    </p>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const app = document.querySelector('#testing-duration-app');

            if (!app) {
                return;
            }

            const allDurations = @js($testingDurations->values());
            const searchInput = app.querySelector('[data-testing-search]');
            const filterSelect = app.querySelector('[data-testing-filter]');
            const tableBody = app.querySelector('[data-testing-table-body]');
            const cardList = app.querySelector('[data-testing-card-list]');
            const countLabel = app.querySelector('[data-testing-count]');
            const pagination = app.querySelector('[data-testing-pagination]');
            const perPage = 15;
            let currentPage = 1;
            let requestTimer = null;
            let filteredDurations = allDurations;
            ...
            const renderPage = (page = 1, shouldScroll = false) => {
                currentPage = page;
                const total = filteredDurations.length;
                const from = total ? (currentPage - 1) * perPage + 1 : 0;
                const to = Math.min(currentPage * perPage, total);
                const pageItems = filteredDurations.slice(from ? from - 1 : 0, to);
            ...
            const filterDurations = () => {
                ...
                filteredDurations = allDurations.filter((item) => {
                    const matchesSearch = keyword === '' || item.search.includes(keyword);
            ...
            const scheduleSearch = () => {
                ...
                requestTimer = setTimeout(filterDurations, 60);
            };

            const renderPagination = () => {
                pagination.innerHTML = '';
                const lastPage = Math.ceil(filteredDurations.length / perPage);
            ...
            const renderTableRow = (item, index) => `
                <tr class="transition odd:bg-white even:bg-[#f8fafc] hover:bg-[#fff8e1]">
                    <td class="px-3 py-4 align-top text-sm font-semibold text-[#71717a]">${((currentPage - 1) * perPage) + index + 1}</td>
                    <td class="px-3 py-4 align-top">
                        <p class="text-[15px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(item.name)}</p>
                    </td>
                    <td class="px-3 py-4 align-top">
                        <p class="text-[14px] font-medium leading-6 text-[#4b5563]">${escapeHtml(item.category)}</p>
                    </td>
                    <td class="px-3 py-4 align-top">
                        <p class="text-[14px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(item.duration)} hari kerja</p>
                    </td>
                </tr>
            `;
            ...
            const renderCard = (item) => `
                <div class="rounded-[8px] border border-black/10 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                    <div class="grid gap-1.5">
                        <p class="text-[13px] font-medium leading-5 text-[#4b5563]">${escapeHtml(item.category)}</p>
                        <p class="text-[13px] font-semibold leading-5 text-[#1f2937]">${escapeHtml(item.duration)} hari kerja</p>
                    </div>
                    <p class="mt-3 text-[15px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(item.name)}</p>
                </div>
            `;
            ...
            filterSelect.addEventListener('change', filterDurations);
            filterDurations();
        });
    </script>
@endpush
