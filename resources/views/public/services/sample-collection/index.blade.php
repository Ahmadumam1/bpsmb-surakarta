...
@section('title', 'Jasa Pengambilan Contoh | BPSMB Surakarta')
@section('meta_description', 'Informasi biaya jasa pengambilan contoh BPSMB Surakarta untuk barang ekspor, sertifikasi mutu, dan sertifikasi produk.')
...
@section('content')
            ...
            <div class="flex items-center">
                <div class="max-w-3xl">
                    ...
                    <p class="mt-3 max-w-2xl text-[13px] leading-6 text-[#4b5563] sm:mt-5 sm:text-[16px] sm:leading-7">
                        Daftar biaya pengambilan contoh untuk kebutuhan ekspor dan sertifikasi.
                    </p>
    ...
    <section class="bg-[#f5f5f7]">
        <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
            <div id="sample-collection-app"
                class="mt-6 overflow-hidden rounded-[8px] border border-black/10 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.08)]">
                <div class="border-b border-black/10 p-4 sm:p-5">
                    <div>
                        <label class="relative block">
                            <span class="sr-only">Cari uraian pengambilan contoh</span>
                            ...
                            <input type="search" data-sample-search value="{{ $search }}" placeholder="Cari uraian..."
                    ...
                    <p class="mt-3 text-sm font-medium text-[#71717a]" data-sample-count>
                        Menampilkan 0 data.
                    </p>
                </div>

                <div data-sample-results>
                    <div class="service-table-compact sample-table-compact overflow-x-auto">
                        <table class="w-full min-w-[500px] table-fixed text-left sm:min-w-[620px]">
                            <thead class="sticky top-0 z-10 bg-[#08236f] text-white">
                                <tr>
                                    ...
                                    <th class="w-[220px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em] sm:w-[300px]">Uraian</th>
                                    <th class="w-[105px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Sampel</th>
                                    <th class="w-[120px] px-3 py-4 text-[12px] font-semibold uppercase tracking-[0.12em]">Biaya</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-black/10" data-sample-table-body></tbody>
                        </table>
                    </div>

                    <div class="hidden" data-sample-card-list></div>
                </div>

                <div class="border-t border-black/10 px-4 py-4 sm:px-5">
                    <div class="flex flex-nowrap items-center justify-start gap-1.5 overflow-x-auto sm:justify-center sm:gap-2" data-sample-pagination></div>
                </div>
            </div>

            @include('public.services.partials.related-services', ['current' => 'sample-collection'])
            ...
            <div class="mt-5 grid gap-3 rounded-[8px] border border-black/10 bg-[#08236f] p-3 text-white shadow-[0_12px_32px_rgba(8,35,111,0.14)] sm:mt-6 sm:gap-4 sm:p-5 sm:shadow-[0_20px_56px_rgba(8,35,111,0.16)] lg:grid-cols-[1fr_auto] lg:items-center">
                <div>
                    <p class="text-[15px] font-semibold leading-snug sm:text-[18px]">Perlu konfirmasi pengambilan contoh?</p>
                    <p class="mt-1.5 max-w-2xl text-[12px] leading-5 text-white/72 sm:text-[13px] sm:leading-6">
                        Hubungi petugas layanan untuk memastikan jadwal, lokasi, dan kebutuhan pengambilan contoh.
                    </p>
                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const app = document.querySelector('#sample-collection-app');
            ...
            const searchInput = app.querySelector('[data-sample-search]');
            const tableBody = app.querySelector('[data-sample-table-body]');
            const cardList = app.querySelector('[data-sample-card-list]');
            const countLabel = app.querySelector('[data-sample-count]');
            const pagination = app.querySelector('[data-sample-pagination]');
            const perPage = 15;
            let currentPage = 1;
            ...
            const renderTableRow = (fee, index) => `
                <tr class="transition odd:bg-white even:bg-[#f8fafc] hover:bg-[#fff8e1]">
                    ...
                    <td class="px-3 py-4 align-top">
                        <p class="text-[15px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(fee.description)}</p>
                    </td>
                    <td class="px-3 py-4 align-top">
                        <p class="text-[14px] font-medium leading-6 text-[#4b5563]">${escapeHtml(fee.sample_label)}</p>
                    </td>
                    <td class="px-3 py-4 align-top">
                        <p class="text-[14px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(fee.formatted_fee)}</p>
                    </td>
                </tr>
            `;
            ...
            const renderCard = (fee) => `
                <div class="rounded-[8px] border border-black/10 bg-white p-4 shadow-[0_10px_30px_rgba(15,23,42,0.05)]">
                    <div class="grid gap-1.5">
                        <p class="text-[13px] font-medium leading-5 text-[#4b5563]">${escapeHtml(fee.sample_label)}</p>
                        <p class="text-[13px] font-semibold leading-5 text-[#1f2937]">${escapeHtml(fee.formatted_fee)}</p>
                    </div>
                    <p class="mt-3 text-[15px] font-semibold leading-6 text-[#1f2937]">${escapeHtml(fee.description)}</p>
                </div>
            `;
