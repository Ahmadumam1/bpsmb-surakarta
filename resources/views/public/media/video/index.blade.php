...
@section('title', 'Video | BPSMB Surakarta')
@section('meta_description', 'Galeri video kegiatan, sosialisasi, dan dokumentasi BPSMB Surakarta.')
...
@section('content')
            ...
            <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p class="mt-3 max-w-2xl text-[13px] leading-6 text-[#4b5563] sm:mt-5 sm:text-[16px] sm:leading-7">
                        Dokumentasi video kegiatan dan layanan BPSMB Surakarta.
                    </p>
                    <div class="mt-4 h-0.5 w-14 rounded-full bg-[#d4af37] sm:mt-7 sm:h-1 sm:w-24"></div>
                </div>

                <form id="video-search-form" action="{{ route('media.video.index') }}" method="GET"
    ...
    <section class="bg-white">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
            <div id="video-results" class="transition-opacity duration-300 ease-out" aria-live="polite">
            </div>
        </div>
    </section>
@endsection
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const allVideos = @js($videos->values());
            const perPage = 8;
            let timer;
            let currentPage = 1;
            ...
            const filterVideos = () => {
                ...
                filteredVideos = keyword
                    ? allVideos.filter((video) => video.search.includes(keyword))
                    : allVideos;

                currentPage = Math.min(Math.max(currentPage, 1), Math.max(Math.ceil(filteredVideos.length / perPage), 1));
                renderVideos();
            };

            const renderVideos = (shouldScroll = false) => {
                const from = (currentPage - 1) * perPage;
                const pageItems = filteredVideos.slice(from, from + perPage);
            ...
            const renderCard = (video) => `
                <article class="overflow-hidden rounded-[8px] border border-black/5 bg-white text-[#1d1d1f] shadow-[0_8px_24px_rgba(15,23,42,0.10)] transition-transform transition-colors transition-shadow hover:-translate-y-1 hover:border-black/10 hover:shadow-[0_18px_42px_rgba(15,23,42,0.16)]">
                    <a href="${escapeHtml(video.embed_url || '#')}" data-video-modal-open data-video-embed-url="${escapeHtml(video.embed_url)}" data-video-title="${escapeHtml(video.title)}">
                        <span class="relative block h-48 overflow-hidden bg-[#f5f5f7] sm:h-56">
                            <img src="${escapeHtml(video.image_url)}" alt="${escapeHtml(video.title)}" class="h-full w-full object-cover" loading="lazy">
                            <span class="absolute inset-0 grid place-items-center bg-black/20 text-white">
                                <span class="grid h-14 w-14 place-items-center rounded-full bg-white/92 text-[#08236f] shadow-lg">
                                    <svg class="ml-0.5 h-6 w-6" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5.5v13l10-6.5-10-6.5Z" /></svg>
                                </span>
                            </span>
                        </span>
                        <div class="p-4 sm:p-6">
                            <div class="flex items-center justify-between gap-4">
                                <span class="rounded-[5px] bg-[#f5f5f7] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#08236f]">${escapeHtml(video.category)}</span>
                                ${video.is_featured ? '<span class="text-[12px] font-semibold text-[#d4af37]">Featured</span>' : ''}
                            </div>
                            <h2 class="mt-4 line-clamp-2 text-[18px] font-semibold leading-[1.22] tracking-[0.231px] text-[#1d1d1f] sm:mt-5 sm:text-[21px] sm:leading-[1.19]">${escapeHtml(video.title)}</h2>
                            <p class="mt-3 line-clamp-3 text-[14px] leading-[1.43] tracking-[-0.224px] text-[#7a7a7a]">${escapeHtml(video.description)}</p>
                            <span class="mt-5 inline-block text-[14px] leading-[1.29] tracking-[-0.224px] text-[#d4af37]">Tonton Video &rarr;</span>
                        </div>
                    </a>
                </article>
            `;
                ...
                <div class="grid min-h-[320px] w-full place-items-center rounded-[8px] border border-black/10 bg-[#f5f5f7] p-8 text-center">
                    <div class="w-full max-w-2xl">
                        <div class="mx-auto grid h-14 w-14 place-items-center rounded-full bg-white text-[#08236f] shadow-[0_14px_36px_rgba(15,23,42,0.08)]">
                            <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                        </div>
            ...
            const renderPagination = () => {
                const lastPage = Math.ceil(filteredVideos.length / perPage);

                if (lastPage <= 1) return '';

                return `<div class="mt-8 flex flex-wrap items-center justify-center gap-1.5 sm:mt-10 sm:gap-2" data-video-pagination>${paginationRange(currentPage, lastPage).map((page) => {
                    ...
                    return `<button type="button" data-video-page="${page}" class="grid h-10 min-w-10 place-items-center rounded-[8px] border text-[14px] font-semibold transition sm:h-11 sm:min-w-11 sm:text-[15px] ${isActive ? 'border-[#d4af37] bg-[#d4af37] text-white' : 'border-black/10 bg-white text-[#4b5563] hover:border-[#08236f] hover:text-[#08236f]'}">${page}</button>`;
                }).join('')}</div>`;
            };
            ...
            results.addEventListener('click', (event) => {
                const button = event.target.closest('[data-video-page]');
                ...
                event.preventDefault();
                currentPage = Number(button.dataset.videoPage || 1);
                renderVideos(true);
            });
